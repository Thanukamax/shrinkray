...
fn compress(compression: Compression, data: &[u8]) -> Result<Vec<u8>> {
    ...
    let compressed = match compression {
        // shrinkray-patch (v0.6.2): upstream uses Compression::fast() (level 1)
        // which bloats rewrites ~13% vs UE-cooked paks (UE cooker uses ~level 9).
        // We swap to best() so rewrites stay at size parity with the original
        // cook. Worst case the encoder is ~4x slower per block — fine for
        // shrinkray's "click apply, wait a few seconds" UX. Upstream would
        // accept a builder-level knob; we vendor the patch until that lands.
        Compression::Zlib => {
            let mut compress =
        ...
        // zstd's `encode_all(data, 0)` uses the default level (3). Bump to
        // 19 (near-max) for the same size-parity reason as zlib above.
        Compression::Zstd => zstd::stream::encode_all(data, 19)?,
